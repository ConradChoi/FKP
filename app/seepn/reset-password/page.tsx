...
// Design Ref: app/supplier/reset-password/page.tsx (SUP-07, mirrored per screen-spec §3.6 BY-07).
...
export default function SeepnResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false)
    setDone(true)
    setTimeout(() => router.push('/seepn/login'), 1500)
  }
  ...
  if (done) {
    return (
      <AuthShell title="비밀번호가 변경되었습니다">
        ...
        <Link href="/seepn/login" className="mt-4 block text-center text-body-sm text-primary-600 hover:underline">
          로그인하기
        </Link>
      </AuthShell>
    )
