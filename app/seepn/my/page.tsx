...
// buyer_bookmark, seepn_inquiry, buyer_saved_comparison, partner_review via the buyer's own RLS).
// The company name is not rendered (no such data is collected).
...
export default async function SeepnMyHomePage() {
  ...
  const [{ data: account }, { count: bookmarkCount }, { count: inquiryCount }, { data: recentBookmarks }, { data: recentInquiries }, { count: comparedCount }, deals, { count: reviewCount }] = await Promise.all([
    ...
    supabase.from('partner_review').select('id', { count: 'exact', head: true }),
  ])
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        <StatCard value={`${reviewCount ?? 0}개`} label="작성 리뷰" />
