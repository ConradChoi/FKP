...
> **[2026-09-12 확인] P6(SUP-15 파트너 대시보드, `20260912100000`) 사전점검 완료 — 신규 privacy review 문서 불요.** 이 절의 원칙이 그대로 이행됐고, **"문의수신 건수"(`get_own_partner_inquiry_count()`)에도 동일 패턴이 승계**됐다: 인자 없음·`integer` 1개 반환·`stable`/`security definer`/`search_path=''`·`authenticated`만 EXECUTE(= `get_own_partner_bookmark_count()`와 속성 동일), 시계열·상태별 breakdown·드릴다운 **없음**(화면정의서 D-D4/D-D5), 자기집계 조회 비감사(PR-15 승계). `seepn_inquiry_partner`에는 여전히 **buyer 자기조회 정책 1개뿐이고 파트너용 SELECT 정책은 없다**(§5.3(g) "파트너를 향한 본문 읽기 경로 0건"은 유지 — 이 SECURITY DEFINER 함수가 파트너 측 유일한 읽기 경로이며 본문·발신자·문의 id를 일절 반환하지 않는다). 미검증 파트너 카드 문구도 "검증이 완료되면 집계가 시작됩니다" 수준의 일반 안내이며 반려 사유 등 상태배너 정보를 재노출하지 않는다. 회귀 테스트는 `supabase/tests/seepn_buyer_regression.sql` §j(j1~j5).
