...
| ID | 질문 | 서비스기획자 권고(기본값) |
|---|---|---|
| **OQ-D1** | 미검증 파트너에게도 대시보드(0건)를 그대로 노출할지, 검증 완료 전에는 탭 자체를 숨기거나 비활성화할지 | **노출 권고(D-D6)** — 최소 개수 게이트를 안 두는 D-13②의 정신과 일관되고, "검증하면 이게 채워진다"는 동기부여 효과가 있음. 단 최종 판단은 product-manager |
| **OQ-D2** | 문의수신 건수를 "전체 누적"으로 할지, "미종결(`new`+`in_progress`)만" 셀지 | **전체 누적 권고** — 관심수(`get_own_partner_bookmark_count`)도 상태 구분 없는 "현재 총량" 개념이라 두 카드의 의미론을 맞추는 게 낫고, 상태별 분리는 §4.3에서 이미 배제한 이유(추론 위험)와도 상충 |
| **OQ-D3** | `/supplier/profile` 기본 랜딩을 `/basic`에서 `/dashboard`로 바꾸는 것에 대한 최종 승인 — 기존 파트너들이 "로그인하면 바로 프로필 입력 화면"에 익숙해져 있을 수 있음 | **변경 권고(D-D2)** — SP-15가 요구하는 "재로그인 유인"의 실질적 유일한 구현 수단이므로. 다만 이미 가입된 파트너 대상 UX 변화이므로 product-manager 확인 |
